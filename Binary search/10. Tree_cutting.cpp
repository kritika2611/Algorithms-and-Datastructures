...
remain intact). Mirko then takes the parts that were cut off. For example, if the tree row contains trees with 
heights of 20, 15, 10, and 17 metres, and Mirko raises his sawblade to 15 metres, the remaining tree heights after 
cutting will be 15, 15, 10, and 15 metres, respectively, while Mirko will take 5 metres off the first tree and 2 
metres off the fourth tree (7 metres of wood in total).

Mirko is ecologically minded, so he doesn‟t want to cut off more wood than necessary. That‟s why he wants to set 
his sawblade as high as possible. Help Mirko find the maximum integer height of the sawblade that still allows him 
...
long long int woods_collected(long long int n,long long int *arr, long long int H){
	long long woods=0;
	for(int i=0;i<n;i++){
		if(arr[i]>H)
			woods+=arr[i]-H;
...
int main(){
	...
	//binary search
	long long int s=0, e=max;
	long long int ans=-1;
	while(s<=e){
		long long int mid=(s+e)/2;
		long long int woods=woods_collected(n,arr,mid);
		if(woods>=m){
			s=mid+1;
			if(ans==-1 || mid>ans)
				ans=mid;
